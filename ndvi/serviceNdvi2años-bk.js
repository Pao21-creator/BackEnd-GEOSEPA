...
'14/09', '30/09', '16/10', '01/11', '17/11', '03/12'];
...
async function obtenerValorNdviPunto(imagenMasCercana, punto) {
  ...
  try {
    ...
    if (ndviValue !== null && ndviValue !== undefined) {
      ...
      if (!valoresNdviPunto.includes(ndviAjustado)) valoresNdviPunto.push(ndviAjustado);
    } else {
      // Si no es válido, agregar null al array
      valoresNdviPunto.push(null);
      console.log('No se encontró un valor válido de NDVI en el punto:', punto);
    }
  } catch (error) {
    // Si hay un error, agregar null al array
    valoresNdviPunto.push(null);
...
async function graficoNdvi2años(provincia, año, punto) {
  try {
    // Llamamos a la función de autenticación
    await authenticate();

    const prov = ee.FeatureCollection('projects/geosepa/assets/' + `${provincia}`);
    ...
    for (let fecha of fechasReferencia) {
      promises.push(
        obtenerImagenValida(dataset, fecha, prov, año).then(imagenMasCercana => {
          if (imagenMasCercana !== null) {
            if(!fechaGrafico.includes(fecha))fechaGrafico.push(fecha);
            obtenerValorNdviPunto(imagenMasCercana, punto);
          } else {
...
module.exports = { graficoNdvi2años };
