...
async function graficoNdviDepa(provincia, localidad, año, punto) {
  try {
    ...
    const departSeleccionado = departamentos.filter(ee.Filter.and(
      ee.Filter.eq('PROVINCIA', provincia), // Filtrar por provincia
      ee.Filter.eq('DEPTO', localidad)  // Filtrar por departamento
    ));
    ...
    var dataset = ee.ImageCollection('MODIS/061/MOD13Q1')
      ...
      .filterBounds(departSeleccionado). filter(ee.Filter.calendarRange(año, año, 'year'));
